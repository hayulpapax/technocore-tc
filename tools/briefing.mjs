...
const out = [
  ...
  '',
  // The age, and what it means — because on its own it reads as staleness and is not.
  // The census is scheduled for 02:15 UTC and has never once run then: fifteen scheduled
  // runs, every one 4.7-6.8 hours late, landing 16-18 KST. A reader in the morning
  // therefore always sees a figure 15-18 hours old, and that IS the newest one there is.
  // Whether a given reader has already seen it is a question about that reader's own
  // schedule, which this page cannot know and does not answer; it states when the
  // measurement was taken and leaves the rest to whoever is reading.
